...
void Digrafo::pushNodo(Lugar l){pilaNodos.push(l);nodosArr[nNodos++]=l;cout<<"Push: "<<l.etiqueta<<endl;}
Lugar Digrafo::popNodo(){if(pilaNodos.empty()){Lugar l={-1,"VACIA"};return l;}Lugar l=pilaNodos.top();pilaNodos.pop();return l;}
void Digrafo::agregarArista(int u,int v,float w){
    while(nNodos<=max(u,v)){Lugar l;l.id=nNodos;l.etiqueta="Nodo_"+to_string(nNodos);pushNodo(l);}
    matAdj[u][v]=w;
}
void Digrafo::mostrar(){
    cout<<"=== DIGRAFO (ADT Pila) | Tope: ";
    if(!pilaNodos.empty())cout<<pilaNodos.top().etiqueta;else cout<<"VACIA";cout<<" ==="<<endl;
...
void Digrafo::guardarSalidas(){
    ...
    guardarSalidaGrafo("Digrafo_V5_ADT_Pila",nds,arr);
}
