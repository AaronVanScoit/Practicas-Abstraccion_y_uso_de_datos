...
Digrafo::Digrafo():nNodos(0),nAristas(0){
    for(int i=0;i<MAXN;i++) for(int j=0;j<MAXN;j++) matAdj[i][j]=0;
}
void Digrafo::agregarArista(int u,int v,float w){
    nNodos=max(nNodos,max(u,v)+1);
    matAdj[u][v]=(int)w; // Solo dirección u->v (dígrafo)
}
void Digrafo::mostrar(){
    cout<<"=== DIGRAFO (int variable) | Nodos: "<<nNodos<<" ==="<<endl;
    cout<<"Matriz de adyacencia dirigida:"<<endl;
    for(int i=0;i<nNodos;i++){
        for(int j=0;j<nNodos;j++) cout<<matAdj[i][j]<<"\t";
        cout<<endl;
    }
    cout<<"Aristas dirigidas:"<<endl;
    for(int i=0;i<nAristas;i++)
        cout<<"  "<<aristas[i].nodoInicial<<" --> "<<aristas[i].nodoFinal
            <<" ["<<aristas[i].aristaConexion<<"] costo:"<<aristas[i].costo<<endl;
}
void Digrafo::dijkstra(int origen){
    ...
    cout<<"Dijkstra desde nodo "<<origen<<":"<<endl;
    for(int i=0;i<nNodos;i++)
        cout<<"  Nodo "<<i<<": "<<(dist[i]==INT_MAX?"INF":to_string(dist[i]))<<endl;
}
...
void Digrafo::guardarSalidas(){
    ...
    guardarSalidaGrafo("Digrafo_V1_DatoBase_Variable",nds,arr);
}
