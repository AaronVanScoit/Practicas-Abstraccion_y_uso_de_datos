#include "Lib_grafo.h"
#include <climits>
Grafo::Grafo():nNodos(0),nAristas(0){
...
void Grafo::mostrar(){
    cout<<"=== GRAFO (int arreglo) | Nodos: ";
    for(int i=0;i<nNodos;i++)cout<<nodosArr[i]<<" "; cout<<endl;
...
void Grafo::dijkstra(int origen){
    ...
    for(int c=0;c<nNodos-1;c++){
        ...
        for(int v=0;v<nNodos;v++) if(matAdj[u][v]&&!vis[v]&&dist[u]+matAdj[u][v]<dist[v]) dist[v]=dist[u]+matAdj[u][v];
    }
    cout<<"Dijkstra desde "<<origen<<":"<<endl;
    for(int i=0;i<nNodos;i++) cout<<"  Nodo "<<nodosArr[i]<<": "<<(dist[i]==INT_MAX?"INF":to_string(dist[i]))<<endl;
}
...
void Grafo::guardarSalidas(){
    ...
    guardarSalidaGrafo("Grafo_V2_DatoBase_Arreglo",nds,arr);
}
