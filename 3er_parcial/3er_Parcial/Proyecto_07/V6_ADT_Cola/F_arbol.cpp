...
void Arbol::recorridoCola(){
    if(raiz==-1){cout<<"Arbol vacio"<<endl;return;}
    while(!colaRecorrido.empty()) colaRecorrido.pop();
    queue<int> q; q.push(raiz);
    cout<<"Recorrido BFS (ADT Cola): ";
    while(!q.empty()){
        int u=q.front();q.pop();
        colaRecorrido.push(nodos[u].dato);
        cout<<"["<<nodos[u].dato.id<<":"<<nodos[u].dato.etiqueta<<"] ";
        if(nodos[u].izq!=-1) q.push(nodos[u].izq);
        if(nodos[u].der!=-1) q.push(nodos[u].der);
    }
    cout<<endl<<"Frente de cola: "<<colaRecorrido.front().etiqueta<<endl;
}
void Arbol::mostrar(){
    cout<<"=== ARBOL (ADT Cola) | Nodos: "<<nNodos<<" ==="<<endl;
    recorridoCola();
}
...
void Arbol::guardarSalidas(){
    vector<Lugar> nds;for(int i=0;i<nNodos;i++) nds.push_back(nodos[i].dato);
    ...
    guardarSalidaGrafo("Arbol_V6_ADT_Cola",nds,arr);
}
